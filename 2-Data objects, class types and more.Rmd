...
Don’t forget the <code>,</code> after <code>&"quot;"Sri Lanka"&quot;"</code>! This tells R that you want to find ALL rows in which <code>country == "Sri Lanka"&quot;"</code> and to include ALL columns in the dataset. (So leaving a slot empty defaults to include all.) If you only wanted the first column, the you could type <code>gapminder[gapminder$country == &quot;Sri Lanka&quot;, 1]</code>. Note that in R, <code>=</code>is equivalent ot `<-` and is used for assignment, i.e. to create a variable such as <code>x = 5</code>. On the other hand, <code>==</code> is used for equality testing, i.e. if we want to confirm that the variable <code>x</code> we just created is, in fact, equal to 5, we could type <code>x==5</code>. The console should return <code>TRUE</code>. Try it!
...
Since we only want a single variable, this returns a <code>vector</code> listing all observations of Sri Lankan GDP per capita over the years included in the dataset. This isn’t very useful because we don’t know what years are associated with each observation. Let’s pull out yearly data too.</p>
...
<p>Here we create a list of column names we want to pull out of the dataset using <code>c()</code> which combines values into a vector or list much like we did last week. Note that you can use either a list of column names or a list of numerical indexes (e.g. Use c(1,3,5) to select the first, third, and fifth columns, or use c(1:5) to select columns one through five).
...
Ok, what if I want to know the average GDP per capita for all countries or for all years? `aggregate()` can help with that.
...
Want to see how countries ranks? Let's try `order()`.
...
This list the countries by GDP in ascending order. What about descending order?
...
We've now covered some of the basics of indexing and subsetting. I recommend these tutorials on <ahref="http://swcarpentry.github.io/r-novice-gapminder/06-data-subsetting/">data subsetting</a>, <ahref="http://swcarpentry.github.io/r-novice-gapminder/05-data-structures-part2/">data manipulation</a> to help solidfy and extend your understanding. 
...
As you can see there are lots of potential ways to do this. Many more than I will show here.

- What do the plots tell you?

Let's try some more plots. What it I wanted to examine the distribution of the GDP, life expectancy, and population variables?
...
- What do these plots tell you?
  ...
  For now we won't worry about formatting these plots. Next week we'll start looking at `ggplot` and we'll customize some formatting using that package.
